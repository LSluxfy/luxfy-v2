...
let lastAgentIdScanned: string | null = null;
let isInitializing = false;
let isClientReady = false;

const phoneToAgentId = new Map<string, string>();
let client: pkg.Client = createClient();
initializeClient();
...
async function handleMessage(message: pkg.Message) {
  ...
  try {
    ...
    if (trainingData.about) contextParts.push(`Sobre a empresa: ${trainingData.about}`);
    if (trainingData.products_services) contextParts.push(`Produtos e serviços: ${trainingData.products_services}`);
    if (trainingData.faq) contextParts.push(`FAQ: ${trainingData.faq}`);
...
app.listen(port, '0.0.0.0', () => {
  console.log(`[API] Backend rodando em ${process.env.URL}`);
});
